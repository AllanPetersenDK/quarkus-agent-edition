...
- In-memory stores are still used where the book concept matters more than infrastructure, but session state now has an H2-backed persistence seam.
...
- Demo: calculator, clock, fake embeddings, in-memory vector store, H2-backed session state, code generation placeholder.
- Production seam: `OpenAiLlmClient`, code execution, and external search integrations.
