...
- Pattern 1 is `beforeLlm` request optimization with a first-class sliding-window preview seam, Pattern 2 is persisted session continuity, and Pattern 3 is structured long-term memory with cross-session ranking and retrieval.
- `after_run` is the bridge into compact memory persistence, and explicit memory search remains a tool rather than a hidden side effect.
- The long-term memory record is intentionally small but structured: the repo now stores problem-solving memory with task summary, approach, result, and lightweight correctness/error fields when available, and retrieval ranks those fields instead of treating memory as a flat string dump. Dedup is also structured, so exact and near-duplicate writes are suppressed before they land in the store. The runtime seam is now JDBC-backed and vector-like, powered by embeddings rather than only an in-memory demo cache, and retrieval still happens via in-process scoring rather than a dedicated vector index.
- Chapter 6 now also includes persisted pending confirmations and a small explicit retrieval alias (`recall-memory`) alongside the existing conversation-search tool, while auto-injection remains a secondary convenience path that sits beside explicit retrieval. That auto-injection now lives in a hidden/internal `process_llm_request`-style request-prep seam and now shows up as a small request-prep trace entry, so the builder compatibility path stays available without making the model see a new public tool.
...
The plan state is intentionally session/context-based, and replan is driven by the reflection outcome instead of a separate workflow engine.
