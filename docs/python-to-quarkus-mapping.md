...
- Pattern 1 is `beforeLlm` request optimization, Pattern 2 is persisted session continuity, and Pattern 3 is structured long-term memory with cross-session ranking and retrieval.
...
- The long-term memory record is intentionally small but structured: the repo now stores problem-solving memory with task summary, approach, result, and lightweight correctness/error fields when available, and retrieval ranks those fields instead of treating memory as a flat string dump. Dedup is also structured, so exact and near-duplicate writes are suppressed before they land in the store.
- Chapter 6 now also includes persisted pending confirmations and a small explicit retrieval alias (`recall-memory`) alongside the existing conversation-search tool, while auto-injection remains a secondary convenience path that sits beside explicit retrieval.
