...
- The first product lane now exists under `dk.ashlan.agent.product` and `POST /api/v1/assistants/query`. It is a small, conversation-aware document/knowledge assistant built on RAG, memory, planning, reflection, and simple observability metadata. Phase 2 adds JDBC-backed conversation persistence and a small operator inspection lane, while auth/roles/OIDC/tenancy are intentionally phase 3 work. Chapter demos still exist for the book companion story, but they are no longer the most natural product entrypoint.
