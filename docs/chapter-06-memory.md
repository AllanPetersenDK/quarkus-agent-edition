...
- `ConversationSearchTool` and `RecallMemoryTool` are the explicit memory retrieval tools, while automatic memory injection now routes through a hidden/internal `process_llm_request`-style request-prep seam and still keeps the builder fallback for compatibility. It is tool-style request preparation, not a normal model-vælgbar tool in the runtime registry.
...
- The pure task-memory ranking helper now has a tiny Quarkus Cache wrapper in the JDBC-backed retrieval path. It only caches deterministic scores for stable `TaskMemory` + query inputs; memory writes, session state, and full runs are still never cached.
